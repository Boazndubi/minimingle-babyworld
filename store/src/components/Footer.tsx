...
import { Baby, Phone, Mail, MapPin, MessageCircle, Camera, Music2, ChevronUp } from "lucide-react";
...
export default function Footer() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const atBottom = scrollPosition >= pageHeight - 60;
  ...
  return (
    <>
      {/* Pull tab — always visible, toggles the footer */}
      <button
        onClick={() => setIsOpen((prev) => !prev)}
        className="fixed bottom-0 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1.5 bg-slate-900 text-slate-200 text-xs font-medium px-4 py-2 rounded-t-lg border border-b-0 border-slate-700 hover:bg-slate-800 transition-colors shadow-lg"
      >
        <ChevronUp
          size={14}
      ...
      {/* Collapsible footer */}
      <footer
        className={`bg-slate-900 text-slate-300 overflow-hidden transition-all duration-500 ease-in-out ${
